    ...
    <div class="col-md-12">
        <div class="card card-outline card-success">
            <div class="card-header">
                <h3 class="card-title">Editar Datos Registrados</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.configuracion.update', $configuracion->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                                                ...
                                                @if ($data['type'] === 'select')
                                                    <select name="{{ $field }}" id="{{ $field }}" class="form-control" required>
                                                        @foreach ($data['options'] as $value => $label)
                                                            <option value="{{ $value }}" {{ old($field, $configuracion->$field) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                                        @endforeach
                                                    </select>
                                                @else
                                                    <input type="{{ $data['type'] }}"
                                                           ...
                                                           id="{{ $field }}"
                                                           class="form-control"
                                                           value="{{ old($field, $configuracion->$field) }}"
                                                           required>
                                                @endif
                                            </div>
                                            @error($field)
                                ...
                                <center>
                                    <output id="list">
                                        @if ($configuracion->logo)
                                            <img src="{{ asset('storage/' . $configuracion->logo) }}" width="80%" alt="Logo">
                                        @else
